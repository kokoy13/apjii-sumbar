                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="md:col-span-2 space-y-6">
                            <div>
                                ...
                                <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Masukkan judul berita..." class="w-full rounded-lg border-slate-300 shadow-sm focus:border-apjii-blue focus:ring-apjii-blue">
                                @error('title')
                            ...
                            <div>
                                <label for="slug" class="block text-sm font-medium text-slate-700 mb-1">Slug (Opsional)</label>
                                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="Otomatis dibuat dari judul jika dikosongkan" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-apjii-blue focus:ring-apjii-blue font-mono text-sm">
                                @error('slug')
                                    <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="excerpt" id="excerpt" rows="3" placeholder="Ringkasan singkat berita..." class="w-full rounded-lg border-slate-300 shadow-sm focus:border-apjii-blue focus:ring-apjii-blue">{{ old('excerpt') }}</textarea>
                                @error('excerpt')
                                    <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="body" id="body-editor" class="w-full rounded-lg border-slate-300 shadow-sm">{{ old('body') }}</textarea>
                                @error('body')
                                    <p class="mt-1 text-sm text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="space-y-6 bg-slate-50/70 p-5 rounded-xl border border-slate-200/80 h-fit">
                            ...
                            <div>
                                ...
                                <select name="status" id="status" required class="w-full rounded-lg border-slate-300 shadow-sm focus:border-apjii-blue focus:ring-apjii-blue">
                                    <option value="published" {{ old('status', 'published') == 'published' ? 'selected' : '' }}>Diterbitkan (Published)</option>
